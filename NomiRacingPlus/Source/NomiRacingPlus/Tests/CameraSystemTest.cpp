...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FCameraSystemFOVTest, "NomiRacing.CameraSystem.DynamicFOV",
	EAutomationTestFlags::ApplicationContextMask | EAutomationTestFlags::ProductFilter)

bool FCameraSystemFOVTest::RunTest(const FString& Parameters)
{
	UCameraSystem* CameraSystem = NewObject<UCameraSystem>();

	// Test FOV at various speeds
	float BaseFOV = 90.0f;
	float MaxSpeed = 300.0f;

	// At low speed, FOV should be base
	float FOVAtZero = CameraSystem->CalculateDynamicFOV(BaseFOV, 0.0f, MaxSpeed, 0.0f);
	TestEqual(TEXT("FOV at zero speed"), FOVAtZero, BaseFOV);

	float FOVAtLowSpeed = CameraSystem->CalculateDynamicFOV(BaseFOV, 30.0f, MaxSpeed, 0.0f);
	TestEqual(TEXT("FOV at low speed"), FOVAtLowSpeed, BaseFOV);

	// At high speed, FOV should be wider
	float FOVAtHighSpeed = CameraSystem->CalculateDynamicFOV(BaseFOV, 250.0f, MaxSpeed, 0.0f);
	TestTrue(TEXT("FOV at high speed should be wider"), FOVAtHighSpeed > BaseFOV);

	// At max speed, FOV should be at maximum
	float FOVAtMaxSpeed = CameraSystem->CalculateDynamicFOV(BaseFOV, MaxSpeed, MaxSpeed, 0.0f);
	TestTrue(TEXT("FOV at max speed should be wider than high speed"), FOVAtMaxSpeed >= FOVAtHighSpeed);

	// With acceleration, FOV should be even wider
	float FOVWithAccel = CameraSystem->CalculateDynamicFOV(BaseFOV, 200.0f, MaxSpeed, 50.0f);
	float FOVWithoutAccel = CameraSystem->CalculateDynamicFOV(BaseFOV, 200.0f, MaxSpeed, 0.0f);
	TestTrue(TEXT("FOV with acceleration should be wider"), FOVWithAccel >= FOVWithoutAccel);
