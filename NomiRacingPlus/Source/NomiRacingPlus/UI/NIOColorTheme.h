...
public:
	...
	/** Set the AccessibilityManager for color/font adaptation */
	UFUNCTION(BlueprintCallable, Category = "NIO Theme|Accessibility")
	...
	/** Get a color adapted for current accessibility mode */
	UFUNCTION(BlueprintCallable, Category = "NIO Theme|Accessibility")
	FLinearColor GetAccessibleColor(const FLinearColor& OriginalColor) const;

	/** Get the success color for current accessibility mode */
	UFUNCTION(BlueprintCallable, Category = "NIO Theme|Accessibility")
	FLinearColor GetAccessibleSuccessColor() const;

	/** Get the danger color for current accessibility mode */
	UFUNCTION(BlueprintCallable, Category = "NIO Theme|Accessibility")
	FLinearColor GetAccessibleDangerColor() const;

	/** Get a font size scaled by current accessibility preset */
	UFUNCTION(BlueprintCallable, Category = "NIO Theme|Accessibility")
	int32 GetAccessibleFontSize(int32 BaseSize) const;

protected:
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "NIO Theme")
	FNIOUITheme Theme;

	// Accessibility manager reference for color/font adaptation
	UPROPERTY()
